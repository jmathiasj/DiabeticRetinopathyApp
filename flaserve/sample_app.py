...
#         imag = rgb2gray(bloodvessel)
        ...
        n_white_pix = np.sum(bloodvessel == 255)
        print(n_white_pix)
        circles = cv2.HoughCircles(bloodvessel,cv2.HOUGH_GRADIENT,1,6,param1=7,param2=4,
        ...
        # loop over the (x, y) coordinates and radius of the circles
          for (x, y, r) in circles:
            ...
            cv2.circle(bloodvessel, (x, y), r, (255, 0, 255), 2)
            cv2.rectangle(bloodvessel, (x - 5, y - 5), (x + 5, y + 5), (255, 0, 255), -1)

            index = index + 1
        print('Number of MA:', index)
        return index
def rgb2gray(rgb):
...
def extract_ex(img):
  ...
  fundus=optic(fundus)	
  print(fundus)	
  fundus_mask = cv2.imread(os.path.expanduser( "/home/ec2-user/environment/envv/environment/wtproj/flaserve/static/fmask.png"))
...
#     print(prediction)
    ...
    bld=dispbv(imagen)
    exu,imagenbv,microan=threshl(exud,micro,bld)
    exun=str(exud)
    micros=str(micro)
    bldn=str(bld)
...
def prediction():
    global model
    model = load_model('/home/ec2-user/environment/envv/environment/wtproj/flaserve/model.hdf5')
    print(".  Model Loaded  .")
    message = request.get_json(force=True)
    encoded = message['image']
    print(encoded)
